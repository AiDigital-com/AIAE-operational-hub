...
public enum ReportRowSortField implements SearchableField {
	 ...
	 *
	 * <p>Has no grouped form at all: the view computes it with a window function, so any aggregate over
	 * it becomes an aggregate of an aggregate once BigQuery inlines the view, which it rejects. A
	 * grouped read neither selects nor sorts by it - see
	 * {@code BigQueryReportRowService#selectAggregatedMetrics} and {@code #groupedSortField}.
	 */
	AVG_DYNAMIC_RATE_BY_DATE_TACTIC(
			BigQueryAdjustmentsViewColumns.AVG_DYNAMIC_RATE_BY_DATE_TACTIC, true, false),
