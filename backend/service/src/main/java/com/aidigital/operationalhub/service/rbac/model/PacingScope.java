...
public record PacingScope(String kind, List<String> ids) {
	 ...
	 *
	 * <p>Not produced by {@code PacingScopeResolver} yet: §3 of the migration plan gave Pacing a real
	 * {@code campaign_ids} column and an enforced filter on it, so Pacing is ready to receive this shape
	 * — but the Hub has no source yet for WHICH NetSuite campaigns a Client Services person owns
	 * (client_service_manager in BigQuery, plus any Hub-side override — US-104), so this resolver has
	 * nothing correct to put in {@code campaignIds}. Kept here for the contract's symmetry with
	 * dash-gate's {@code normalizeScope}.
	 *
