...
public class BigQueryClientService implements ClientService {
	...
	public Page<ClientModel> searchClients(CurrentUserModel user, SearchCriteria<ClientField> criteria) {
		...
		applyFilters(query, criteria.filters(), visibility.agencyIds());
	 ...
	 * @param visibleAgencyIds visible agency ids; empty means unrestricted
	 */
	void applyFilters(
			BqRequest.Builder query,
			List<FilterCriterion<ClientField>> filters,
			List<Long> visibleAgencyIds) {
		if (filters == null) {
			return;
		}
		for (FilterCriterion<ClientField> filter : filters) {
			switch (filter.field()) {
				case NAME -> applyClientNameFilter(query, filter.value(), visibleAgencyIds);
	...
	/**
	 * Applies a client-name search against both IO-lines advertiser names and reporting mart client
	 * names.
	 *
	 * @param query            the query builder
	 ...
	 * @param visibleAgencyIds visible agency ids; empty means unrestricted
	 */
	void applyClientNameFilter(BqRequest.Builder query, String value, List<Long> visibleAgencyIds) {
		query.whereContainsSubstrOrStructInSubquery(
				ADVERTISER,
				value,
				List.of(AGENCY_ID, ADVERTISER_ID),
				clientResolver.agencyClientRowsForMartClientSearch(visibleAgencyIds, value));
	}
