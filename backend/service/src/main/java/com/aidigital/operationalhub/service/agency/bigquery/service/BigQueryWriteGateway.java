...
public class BigQueryWriteGateway {
	...
	/**
	 * Replaces a table with the result of a query, creating it when it does not exist yet.
	 *
	 * <p>{@code CREATE OR REPLACE TABLE ... AS} rather than a query job with a destination table and
	 * {@code WRITE_TRUNCATE}, which is how the spreadsheet does the same thing: the two are equivalent, and
	 * this one needs nothing of BigQuery that this application cannot already ask for.
	 *
	 * <p>Replace, not append. A dashboard's data source is the answer to one question asked afresh - a second
	 * run that added rows would double every figure on the dashboard.
	 *
	 * @param table the fully-qualified table to write
	 * @param query the query whose result becomes the table
	 * @throws BusinessException OPH_026 when the BigQuery write fails
	 */
	public void replaceTable(String table, String query) {
		try {
			bigQueryWriteClient.execute(
					"CREATE OR REPLACE TABLE `" + table + "` AS\n" + query,
					bigQueryProperties.getTableWriteTimeoutMs());
		} catch (BigQueryExternalException ex) {
			log.error("Failed to write BigQuery table {}", table, ex);
			throw new BusinessException(OperationalHubErrorReason.OPH_026, ex, causeMessage(ex));
		}
	}
