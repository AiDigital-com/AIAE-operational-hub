...
public class PacingExternalException extends RuntimeException {
	...
	/**
	 * Constructs an exception with a descriptive message, an explicit failure reason and Pacing's own
	 * human-readable detail string, safe to forward to the Hub's own caller (used for
	 * {@link PacingFailureReason#UPSTREAM_BAD_REQUEST}, where Pacing's {@code detail} names which part
	 * of the request it rejected).
	 *
	 * @param reason  why the call to Pacing failed
	 * @param message human-readable description (must not contain the shared secret)
	 * @param detail  Pacing's own detail string, safe to forward verbatim; may be null
	 */
	public PacingExternalException(PacingFailureReason reason, String message, String detail) {
		super(message);
		this.reason = reason;
		this.detail = detail;
	}
	...
	/**
	 * Returns Pacing's own detail string, safe to forward verbatim to the Hub's own caller. Only set
	 * for {@link PacingFailureReason#UPSTREAM_BAD_REQUEST}; null for every other reason.
	 *
	 * @return the detail string, or null
	 */
