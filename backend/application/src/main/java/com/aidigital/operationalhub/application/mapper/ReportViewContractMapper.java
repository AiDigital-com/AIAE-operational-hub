...
public class ReportViewContractMapper {
	...
	public ReportViewV1 toV1(HubReportView entity) {
		...
		// Null stays null rather than becoming an empty list: absent means "nobody reordered these", and an
		// empty list would read as "shown in no order at all".
		v1.setColumnOrder(entity.getColumnOrder() == null ? null : split(entity.getColumnOrder()));
	...
	public HubReportView fromUpsert(ReportViewUpsertV1 request) {
		...
		entity.setColumnOrder(joinOrNull(request.getColumnOrder()));
	...
	/**
	 * Joins the column order, keeping absent distinct from empty.
	 *
	 * <p>{@link #join} answers an empty string for nothing, which is right for the selections - a report with
	 * no metrics selected has an empty metric list. It is wrong here: the column order has no empty case, so
	 * an empty string would be a third state nobody means, sitting between "canonical order" and "this
	 * order". Null is the one that means the first.
	 *
	 * @param ids the ids to join
	 * @return the comma-joined value, or null when there is no order to record
	 */
	String joinOrNull(List<String> ids) {
