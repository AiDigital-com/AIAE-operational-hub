...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<OperationalHubApiExceptionResponseV1> handlePacingExternalException(
		...
		// OPH_056's template takes Pacing's own detail string (§6: "the request was rejected because
		// ..." beats a bare "the request was rejected") - every other reason's template is fixed text
		// and formats safely with zero params.
		String detail = exception.getDetail() == null ? "invalid request" : exception.getDetail();
		BusinessException businessException = exception.getReason() == PacingFailureReason.UPSTREAM_BAD_REQUEST
				? new BusinessException(reason, detail)
				: new BusinessException(reason);
		return responseHelper.buildApiError(businessException, status);
	}
