 ...
 * cpm/ctr/avcr ratios (never stored, never writable) and the server-owned audit stamps. It keeps the
 * mart client/campaign identity from the exported row so uploads do not rewrite those values from the
 * Hub campaign. Mirrors {@link com.aidigital.operationalhub.service.agency.model.AdjustmentRowModel}'s non-derived,
...
public class ReportRowXlsxAssembler {
	...
	private static final String[] TEMPLATE_COLUMNS = {
			...
			"campaign_constructed_id", "agency_id", "client", "industry_code", "campaign_name", "channel", "tactic",
	...
	private static final Set<String> NUMERIC_COLUMNS = Set.copyOf(METRIC_COLUMNS);
	...
	void writeHeader(Sheet sheet, List<String> levelTerms) {
		...
		int metadataColumn = TEMPLATE_COLUMNS.length;
		header.createCell(metadataColumn++).setCellValue(ReportRowKey.WORKBOOK_COLUMN);
		header.createCell(metadataColumn++).setCellValue(ReportRowKey.WORKBOOK_SOURCE_DATE_COLUMN);
		header.createCell(metadataColumn++).setCellValue(ReportRowKey.WORKBOOK_SOURCE_CONSTRUCTED_ID_COLUMN);
		for (String metric : METRIC_COLUMNS) {
			header.createCell(metadataColumn++).setCellValue(ReportRowKey.originalMetricColumn(metric));
		}
	}
	...
	void writeRow(Sheet sheet, int rowNum, ReportRowModel row) {
		...
		int metadataColumn = TEMPLATE_COLUMNS.length;
		sheetRow.createCell(metadataColumn++).setCellValue(ReportRowKey.of(row).encoded());
		sheetRow.createCell(metadataColumn++).setCellValue(row.date());
		sheetRow.createCell(metadataColumn++).setCellValue(row.lineItemId());
		for (String metric : METRIC_COLUMNS) {
			Object value = valueFor(metric, row);
			if (value != null) {
				sheetRow.createCell(metadataColumn).setCellValue(((Number) value).doubleValue());
			} else {
				sheetRow.createCell(metadataColumn).setCellValue(ReportRowKey.ORIGINAL_NULL_VALUE);
			}
			metadataColumn++;
		}
	}
	...
	/**
	 * Hides workbook-only metadata columns: row identity, canonical baseline lookup identity, and the
	 * original metric values used by upload diffing.
	 *
	 * @param sheet the sheet to update
	 */
	void hideMetadataColumns(Sheet sheet) {
		int metadataColumnCount = 3 + METRIC_COLUMNS.size();
		for (int i = 0; i < metadataColumnCount; i++) {
			sheet.setColumnHidden(TEMPLATE_COLUMNS.length + i, true);
		}
	}
	 ...
	 *
	 * <p><strong>Only the current vocabulary is accepted.</strong> PDI_105 moved "Client Cost" from
	 * {@code spend} to {@code dynamic_cost} and renamed {@code spend} to "DSP Cost", so a template
	 * downloaded before that rename says "Client Cost" over a column this map now reads as
	 * {@code dynamic_cost}. Deliberately not accommodated: the labels here follow the Reporting table, and
	 * a template is downloaded to be filled in and sent back, not kept. Download a fresh one.
	 *
	...
	Map<String, String> headerAliases() {
		...
		aliases.put(ReportRowKey.WORKBOOK_COLUMN, ReportRowKey.WORKBOOK_COLUMN);
		aliases.put(ReportRowKey.WORKBOOK_SOURCE_DATE_COLUMN, ReportRowKey.WORKBOOK_SOURCE_DATE_COLUMN);
		aliases.put(
				ReportRowKey.WORKBOOK_SOURCE_CONSTRUCTED_ID_COLUMN,
				ReportRowKey.WORKBOOK_SOURCE_CONSTRUCTED_ID_COLUMN);
		for (String metric : METRIC_COLUMNS) {
			aliases.put(
					ReportRowKey.originalMetricColumn(metric),
					ReportRowKey.originalMetricColumn(metric));
		}
		for (List<String> terms : ReportRowXlsxExportAssembler.PLATFORM_LEVEL_TERMS.values()) {
			for (String column : TEMPLATE_COLUMNS) {
				aliases.putIfAbsent(normalizeHeader(labelResolver.displayHeader(column, terms)), column);
			}
		}
		aliases.put("line item name", "line_item_name");
		aliases.put("line item id", "line_item_id");
		aliases.put("insertion order name", "insertion_order_name");
		aliases.put("insertion order id", "insertion_order_id");
		aliases.put("creative name", "campaign_constructed_name");
		aliases.put("creative id", "campaign_constructed_id");
