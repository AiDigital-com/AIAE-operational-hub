...
class ReportViewContractMapperTest {
	...
	@Test
	void shouldCarryAColumnOrderBothWaysTest() {
		// Given: a report whose columns someone has dragged
		HubReportView entity = entity("date", "spend,impressions");
		entity.setCreatedAt(LocalDateTime.of(2026, 1, 1, 10, 0));
		entity.setUpdatedAt(LocalDateTime.of(2026, 1, 2, 11, 30));
		entity.setColumnOrder("spend,impressions,date");

		// When:
		ReportViewV1 v1 = mapper.toV1(entity);

		// Then:
		assertThat(v1.getColumnOrder()).containsExactly("spend", "impressions", "date");
	}

	@Test
	void shouldLeaveAnUntouchedColumnOrderAbsentRatherThanEmptyTest() {
		// Given: a report nobody has reordered - every report saved before the column existed
		HubReportView entity = entity("date", "spend");
		entity.setCreatedAt(LocalDateTime.of(2026, 1, 1, 10, 0));
		entity.setUpdatedAt(LocalDateTime.of(2026, 1, 2, 11, 30));
		...
		// Then: absent, not an empty list. Empty would read as "shown in no order", and the client would have
		// no way left to say "use the canonical order" - which is what every existing report needs it to say.
		assertThat(v1.getColumnOrder()).isNull();
	}

	@Test
	void shouldStoreNoColumnOrderWhenTheRequestCarriesNoneTest() {
		// Given: a save from a report whose columns were never dragged
		ReportViewUpsertV1 request = new ReportViewUpsertV1()
				.name("Report")
				.type(ReportViewTypeEnumV1.BASIC)
				.status(ReportViewStatusEnumV1.SAVED)
				.dimensions(List.of("date"))
				.metrics(List.of("spend"));

		// When:
		HubReportView entity = mapper.fromUpsert(request);

		// Then: null rather than "", so a later read cannot mistake it for an order of no columns
		assertThat(entity.getColumnOrder()).isNull();
	}
}
