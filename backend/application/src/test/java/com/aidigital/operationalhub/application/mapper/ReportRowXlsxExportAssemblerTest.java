...
class ReportRowXlsxExportAssemblerTest {
	...
	void shouldRenderTheFullRawExportSchemaWhenNoColumnsSelectedTest() throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
			...
			assertThat(headerNames).containsExactly(
					...
					"Flight identifier", "Language", "Impressions", "Clicks", "Client Cost", "Starts", "First quartiles",
					"Midpoints", "Third quartiles", "Completions", "Conversions", "Post-click conversions",
					"Post-view conversions", "Dynamic cost", "Link clicks", "Adjusted metrics", "Created at",
					"Created by", "Last modified at", "Last modified by", "Rate type", "Dynamic rate",
					"Avg dynamic rate (by date/tactic)", "Description", "IVT");
		}
	}
	...
	void shouldResolveEveryExportableColumnIncludingDerivedRatiosTest() throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
			...
			assertThat(headerNames).containsExactly(
					...
					"Post-view conversions", "Dynamic cost", "Link clicks", "Adjusted metrics", "Created at",
					"Created by", "Last modified at", "Last modified by", "Rate type", "Dynamic rate",
					"Avg dynamic rate (by date/tactic)", "Description", "Client CPM", "CPC", "CPV", "CTR", "AVCR",
					"IVT");
			Row data = workbook.getSheetAt(0).getRow(1);
	...
	void shouldNotExposeRawColumnIdsInAFullDownloadHeaderTest() throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
			...
			assertThat(headerNames).contains("Adjusted metrics", "Avg dynamic rate (by date/tactic)", "IVT");
		}
	}
	...
	void shouldStateTheReportsOwnTotalsOnTheirOwnSheetTest() throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
			...
			assertThat(sheetTotals.getRow(2).getCell(0).getStringCellValue()).isEqualTo("Client Cost");
			assertThat(sheetTotals.getRow(3).getCell(0).getStringCellValue()).isEqualTo("Client CPM");
