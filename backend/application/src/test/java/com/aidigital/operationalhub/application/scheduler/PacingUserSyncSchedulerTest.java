...
/**
 * Unit tests for {@link PacingUserSyncScheduler}, mirroring {@code NetSuiteSyncSchedulerTest}.
 */
@ExtendWith(MockitoExtension.class)
class PacingUserSyncSchedulerTest {
	...
	@InjectMocks
	private PacingUserSyncScheduler scheduler;
	...
	void shouldRunSyncOnScheduleWhenTheLockIsAcquiredTest() {
		...
		when(pacingUserSyncService.sync()).thenReturn(new PacingUserSyncSummary(1, 1, 0, 0, 0, 1));

		// When:
		scheduler.syncDaily();

		// Verification:
		verify(pacingUserSyncService).sync();
	}
	...
	void shouldSkipSyncWhenTheLockIsHeldByAnotherNodeTest() {
		...
		verify(pacingUserSyncService, never()).sync();
	}
	...
	void shouldSwallowFailureSoTheScheduleKeepsFiringTest() {
		...
		when(pacingUserSyncService.sync()).thenThrow(new RuntimeException("boom"));
