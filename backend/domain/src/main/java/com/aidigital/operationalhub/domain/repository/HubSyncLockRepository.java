...
public interface HubSyncLockRepository extends JpaRepository<HubSyncLock, String> {
	...
	/**
	 * Ensures the named lock row exists before a conditional acquisition attempt. The conflict handler makes
	 * first use of dynamic lock names safe across nodes.
	 *
	 * @param lockName the lock row to create when missing
	 */
	@Modifying
	@Query(
			value = "INSERT INTO hub_sync_lock (lock_name, locked) VALUES (:lockName, false) "
					+ "ON CONFLICT (lock_name) DO NOTHING",
			nativeQuery = true)
	void ensureExists(@Param("lockName") String lockName);
