...
public class HubReportView extends AuditAwareEntity {
	...
	/**
	 * Comma-joined column ids in the order they should be shown; null when nobody has reordered them.
	 *
	 * <p>Null is not the same as empty and carries the meaning: it says no preference was expressed, so the
	 * product's canonical column order applies. It cannot be inferred from {@link #dimensions} instead -
	 * that order is whichever order the checkboxes were ticked in, and reading it as intent would rearrange
	 * every report saved before this column existed.
	 */
