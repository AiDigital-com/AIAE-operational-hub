...
import { aClientPageV1, anAgencyClientV1, anAgencyPageV1, anAgencyV1, aUserV1 } from "@/test/factories";
...
describe("Sidebar", () => {
  ...
  it("should load the clients an expanded agency has past the embedded ones", async () => {
    // Given: an agency with 18 clients, of which the list embeds the first 16
    const embedded = Array.from({ length: 16 }, (_, index) =>
      anAgencyClientV1({ id: index + 1, name: `Client ${String(index + 1).padStart(2, "0")}` }));
    vi.mocked(searchAgencies).mockResolvedValue(anAgencyPageV1({
      content: [anAgencyV1({ id: 42, name: "True Media", clients: embedded, clientsCount: 18 })],
    }));
    vi.mocked(searchClients).mockResolvedValue(aClientPageV1({
      content: [
        { id: 17, name: "St. George's University", agency_id: 42 },
        { id: 18, name: "Syngenta", agency_id: 42 },
      ],
      pageNumber: 2,
      pageSize: 16,
      totalElements: 18,
    ...
    // When: the agency is expanded
    await userEvent.click(await screen.findByRole("link", { name: /True Media/ }));

    // Then: the ones the embedded list could not carry arrive on their own - expanding is the ask, and a
    // second click to see the rest of a list you just opened is the thing the ticket complained about
    expect(await screen.findByRole("link", { name: /St. George's University/ })).toBeInTheDocument();
    // Scoped to the client list's own step, not the agency list's below it, which says "N more" too.
    expect(document.querySelector(".sidebar__clients-more")).toBeNull();

    // And page one was never re-requested: the embedded clients already are page one
    expect(searchClients).toHaveBeenCalledTimes(1);
    expect(vi.mocked(searchClients).mock.calls[0][0]).toBe(2);
  });

  it("should read only what a search for the agency's own name left out", async () => {
    // Given: a search matching the agency's name, so its row carries its ordinary first page of 16 - the
    // server narrows clients only for agencies that matched through a client
    const embedded = Array.from({ length: 16 }, (_, index) =>
      anAgencyClientV1({ id: index + 1, name: `Client ${String(index + 1).padStart(2, "0")}` }));
    vi.mocked(searchAgencies).mockResolvedValue(anAgencyPageV1({
      content: [anAgencyV1({ id: 42, name: "True Media", clients: embedded, clientsCount: 22 })],
    }));
    vi.mocked(searchClients).mockResolvedValue(aClientPageV1({
      content: Array.from({ length: 6 }, (_, index) => ({
        id: index + 17,
        name: `Client ${index + 17}`,
        agency_id: 42,
      })),
      pageNumber: 2,
      pageSize: 16,
    ...
    renderSidebar("/");
    await userEvent.type(screen.getByRole("searchbox", { name: /Search agencies or clients/ }), "true media");
    await screen.findByRole("link", { name: /Client 01/ });

    // And: nothing is requested until asked for - a search expands every result at once
    expect(searchClients).not.toHaveBeenCalled();

    // When: the step is pressed once
    await userEvent.click(await screen.findByRole("button", { name: /6 more/ }));

    // Then: the 16 already on screen are not read again - only the page they left off at
    expect(await screen.findByRole("link", { name: /Client 22/ })).toBeInTheDocument();
    expect(document.querySelectorAll(".sidebar__client-row")).toHaveLength(22);
    expect(document.querySelector(".sidebar__clients-more")).toBeNull();
    expect(searchClients).toHaveBeenCalledTimes(1);
    expect(vi.mocked(searchClients).mock.calls[0].slice(0, 2)).toEqual([2, 16]);
  });

  it("should read the agency's own list when a search matched through a client", async () => {
    // Given: a search matching a client, so the row carries that client alone - not the first page
    vi.mocked(searchAgencies).mockResolvedValue(anAgencyPageV1({
      content: [anAgencyV1({
        id: 42,
        name: "True Media",
        clients: [anAgencyClientV1({ id: 9, name: "Chime" })],
        clientsCount: 22,
      })],
    }));
    vi.mocked(searchClients).mockResolvedValue(aClientPageV1({
    ...
    renderSidebar("/");
    await userEvent.type(screen.getByRole("searchbox", { name: /Search agencies or clients/ }), "chime");
    await screen.findByRole("link", { name: /Chime/ });

    // When: the step, which here offers the agency's list rather than a count of what is missing
    await userEvent.click(await screen.findByRole("button", { name: /Show all 22/ }));

    // Then: read from page one, because appending page two to one matching client would leave a list that
    // is neither the search's answer nor the agency's own
    expect(await screen.findByRole("link", { name: /Client 01/ })).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: /Chime/ })).not.toBeInTheDocument();
    expect(vi.mocked(searchClients).mock.calls[0].slice(0, 2)).toEqual([1, 16]);
    // And the rest stays one page per press
    expect(await screen.findByRole("button", { name: /6 more/ })).toBeInTheDocument();
  });

  it("should not ask for more clients when the embedded list is already all of them", async () => {
    // Given: an agency whose every client is embedded
    vi.mocked(searchAgencies).mockResolvedValue(anAgencyPageV1({
      content: [anAgencyV1({
        id: 42,
        name: "True Media",
        clients: [anAgencyClientV1({ id: 1, name: "Only Client" })],
        clientsCount: 1,
      })],
    }));
    renderSidebar("/");
    ...
    // When:
    await userEvent.click(await screen.findByRole("link", { name: /True Media/ }));
    await screen.findByRole("link", { name: /Only Client/ });
