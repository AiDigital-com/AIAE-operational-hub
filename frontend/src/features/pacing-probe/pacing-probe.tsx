        ...
        <>
          ...
          <section className="pacing-probe__panel">
            ...
            {outcome.body.pacings.length === 0 ? (
              <p>
                Empty. Correct, not broken, when your scope entitles you to nothing — e.g. an
                <code>owners</code> scope with no ids, or a <code>campaigns</code> scope naming
                campaigns this pacing set doesn&apos;t cover. Pacing filters <code>campaigns</code>{" "}
                scopes for real now (§3 of the migration plan); a Client Services user still sees
                nothing today only because the Hub itself doesn&apos;t yet resolve which NetSuite
                campaigns they own — it sends an empty <code>owners</code> scope for them, a
                separate piece of work from §3.
              </p>
            ) : (
              <table className="pacing-probe__table">
                <thead>
