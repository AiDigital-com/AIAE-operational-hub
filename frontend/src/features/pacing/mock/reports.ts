 ...
 *
 * Includes `client`/`campaign_name` from the report row itself: NetSuite campaign names can differ from
 * mart campaign names, and writing the resolved campaign name would make the adjustment miss the row it
 * came from. Deliberately excludes the read-only display columns (`rate_type`, `line_item_description`,
...
export const METRIC_DEFS: MetricDef[] = [
  ...
  { id: "clicks", label: "Clicks", agg: "SUM", description: "Total clicks recorded on the ad." },
  {
    ...
    label: "DSP Cost",
    agg: "SUM",
    description: "Media cost as the DSP reports it; not what the client is billed.",
  },
  ...
  { id: "conversions", label: "Conversions", agg: "SUM", description: "Total tracked conversions." },
  {
    ...
    agg: "SUM",
    description:
      "Conversions the platform attributed to a click, from the delivery mart. Not a breakdown of "
      + "Conversions, which comes from the conversions mart - the two are not expected to add up.",
  },
  {
    ...
    agg: "SUM",
    description:
      "Conversions the platform attributed to a view without a click, from the delivery mart. "
      + "View-through counts run far larger than clicked ones; see Post-click conversions.",
  },
  {
    id: "dynamic_cost",
    label: "Client Cost",
    agg: "SUM",
    description: "What the client is billed, at the rate card's rate; not the DSP media cost.",
  },
  ...
  {
    id: "cpm",
    // What the reporting tool calls it: its own reports output this column as CPM, plainly. It was
    // briefly "Client CPM (delivered)" - PDI_105 moved the name "Client CPM" to the rate card's own rate
    // (`dynamic_rate`), and rather than use the tool's word for the column left behind, the rename
    // invented a qualifier. Two columns still cannot share one name in a picker; this one does not need to.
    label: "CPM",
    agg: "WTD",
    description:
      "What the client's cost worked out to per 1,000 impressions delivered. Weighted: total client "
  ...
  },
  {
    ...
    label: "Client CPM",
    agg: "WTD",
    description: "The rate card's own client CPM. Weighted: total client cost / total billable units.",
  },
  {
    id: "avg_dynamic_rate_by_date_tactic",
    label: "Average Client CPM (by date/tactic)",
    agg: "AVG",
    description: "The client CPM averaged per date and tactic.",
  },
];
